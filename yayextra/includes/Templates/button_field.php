...
foreach ( $option_value_list as $index => $opt ) {
	if ( ! empty( $opt['additionalDescription'] ) && ! empty( $opt['additionalDescription']['isEnabled'] ) && ! empty( $opt['additionalDescription']['description'] ) ) {
		...
		echo '<p class="yayextra-addition-des yayextra-addition-des-swatches-button yayextra-addition-des-button" data-opt-id="' . esc_attr( $data['id'] ) . '" data-opt-val="' . esc_attr( $opt['value'] ) . '">' . wp_kses_post( nl2br($addition_description) ) . '</p>';
	}
}
